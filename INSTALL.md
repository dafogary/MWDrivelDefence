...
### Automated Batch Analysis (Cron Job)

To automatically analyze all pages edited in the last 24 hours and post detailed feedback to talk pages, add this to your server's crontab:

```bash
0 2 * * * cd /path/to/your/mediawiki && php extensions/MWDrivelDefence/analyzePagesForPlainEnglish.php --recent=1 --limit=20 > /dev/null 2>&1
```

- This will run the analysis every day at 2 AM.
- Adjust `--limit=20` as needed for your wiki size.
- You can use `--recent=7` for weekly or `--recent=30` for monthly analysis.
- For more options, see the README batch analysis section.
